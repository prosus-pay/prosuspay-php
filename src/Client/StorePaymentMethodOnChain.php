...
use ProsusPay\Result\StorePaymentMethodOnChain as ResultStorePaymentMethodOnChain;
...
class StorePaymentMethodOnChain extends AbstractStorePaymentMethodClient
{
     ...
     *                        ]
     *
     * @return \ProsusPay\Result\StorePaymentMethodOnChain
     * @throws \JsonException
     *
     */
    public function updatePaymentMethod(string $storeId, string $cryptoCode, array $settings): \ProsusPay\Result\StorePaymentMethodOnChain
    {
        $url = $this->getApiUrl() . 'stores/' . urlencode($storeId) . '/payment-methods/' . self::PAYMENT_TYPE_ONCHAIN . '/' . $cryptoCode;
        $headers = $this->getRequestHeaders();
        $method = 'PUT';
        ...
        if ($response->getStatus() === 200) {
            $data = json_decode($response->getBody(), true, 512, JSON_THROW_ON_ERROR);
            return new \ProsusPay\Result\StorePaymentMethodOnChain($data, $data['cryptoCode']);
        } else {
            throw $this->getExceptionByStatusCode($method, $url, $response);
        }
    }
    ...
    public function previewPaymentMethodAddresses(string $storeId, string $cryptoCode): array
    {
        ...
        if ($response->getStatus() === 200) {
            $addressList = new \ProsusPay\Result\AddressList(
