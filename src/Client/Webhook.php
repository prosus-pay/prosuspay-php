...
use ProsusPay\Result\Webhook as ResultWebhook;
use ProsusPay\Result\WebhookCreated;
use ProsusPay\Result\WebhookDelivery;
use ProsusPay\Result\WebhookDeliveryList;
use ProsusPay\Result\WebhookList;
...
class Webhook extends AbstractClient
{
    ...
    public function createWebhook(
        ...
        if ($secret === '') {
            throw new \InvalidArgumentException('Argument $secret should be NULL (let Prosus Pay auto-generate a secret) or you should provide a long and safe secret string.');
        } elseif ($secret !== null) {
            $data['secret'] = $secret;
        }
     ...
     *
     * Important: due to a bug in Prosus Pay versions <= 1.6.3.0 you need
     ...
     * @param string $prosuspaySigHeader Most likely you will use `$_SERVER['HTTP_ProsusPay-Sig']` for this.
     * @param string $secret The secret that's registered with the webhook in Prosus Pay as a security precaution.
     * @return bool
     */
    public static function isIncomingWebhookRequestValid(string $requestBody, string $prosuspaySigHeader, string $secret): bool
    {
        if ($requestBody && $prosuspaySigHeader) {
            $expectedHeader = 'sha256=' . hash_hmac('sha256', $requestBody, $secret);

            if ($expectedHeader === $prosuspaySigHeader) {
                return true;
            }
        }
        return false;
