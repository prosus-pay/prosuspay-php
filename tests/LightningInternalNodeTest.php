...
final class LightningInternalNodeTest extends BaseTest
{
    ...
    {
        $lightningClient = new \ProsusPay\Client\LightningInternalNode($this->host, $this->apiKey);
        ...
        $this->assertInstanceOf(\ProsusPay\Result\LightningInvoice::class, $lightningInvoice);
        ...
        $this->assertInstanceOf(\ProsusPay\Util\PreciseNumber::class, $lightningInvoice->getAmount());

        // If the lightning invoice is paid, assert the amount received is a PreciseNumber
        if ($lightningInvoice->getAmountReceived()) {
            $this->assertInstanceOf(\ProsusPay\Util\PreciseNumber::class, $lightningInvoice->getAmountReceived());
        }
    }
    ...
    public function testItReceivesLightningPaymentObjectAfterPayingLightningInvoiceWithAllGetters(): void
    {
        ...
        $this->assertInstanceOf(\ProsusPay\Result\LightningPayment::class, $lightningPayment);

        // There is a bug in API that is returning null values on everything except total and fee amounts.
        // Uncomment these lines when the bug is fixed.
        // https://github.com/prosuspay/prosuspay/issues/4229
        ...
        $this->assertInstanceOf(\ProsusPay\Util\PreciseNumber::class, $lightningPayment->getTotalAmount());
        $this->assertInstanceOf(\ProsusPay\Util\PreciseNumber::class, $lightningPayment->getFeeAmount());
    }
    ...
    public function testItCanConnectToALightningNodeAndReturnsLightningNodeConnectionObject(): void
    {
        ...
        try {
            ...
            $this->assertInstanceOf(\ProsusPay\Result\LightningNodeConnection::class, $lightningNodeConnection);
        } catch (\ProsusPay\Client\ProsusPayException $e) {
            die($e->getMessage());
        }
    }
    ...
    public function testItCanGetNodeInformationAndReturnsLightningNodeInformationObject(): void
    {
        ...
        $this->assertInstanceOf(\ProsusPay\Result\LightningNode::class, $lightningNodeInformation);
    ...
    public function testItCanGetChannelsAndReturnsLightningChannelListObject(): void
    {
        ...
        $this->assertInstanceOf(\ProsusPay\Result\LightningChannelList::class, $lightningChannels);

        $this->assertIsArray($lightningChannels->all());

        foreach ($lightningChannels->all() as $channel) {
            $this->assertInstanceOf(\ProsusPay\Result\LightningChannel::class, $channel);
